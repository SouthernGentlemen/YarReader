...
**[Architecture](ARCHITECTURE.md)** · **[Releases](../../releases)** ·
...
## Private series curation

Reading modes, genres, optional cover-page URLs, and explicit series merges can
be supplied in `state/series-curation.json`. The file is runtime state outside
the repository and is validated before use. Start from the synthetic
[`examples/series-curation.json`](examples/series-curation.json) shape; do not
commit a real library inventory.

Run `yar covers` to fetch only the cover sources configured for series present
in the current catalog. Downloaded images and their index remain under
`state/covers/`; an export copies only optimized local WebP assets into its
...
runtime, browser persistence, or absolute machine paths. When JavaScript is
available, bundled no-network scripts progressively enhance the same static
catalog and pages with search, filtering, reading modes, thumbnails, and saved
reading progress; if those scripts are blocked, the links and images remain
directly usable.
...
```sh
yar portable /path/to/removable/YarReader
```
...
This public history was reconstructed from the original YarReader repository
and its tests. The reconstructed commit structure does not assert that each
public commit originally existed independently. Source commits and dates are
recorded in [docs/history/CHANGE-MAP.csv](docs/history/CHANGE-MAP.csv), and the
method and intentional privacy transform are documented in
[docs/RECONSTRUCTION.md](docs/RECONSTRUCTION.md).

## Security and license

Report vulnerabilities privately as described in [SECURITY.md](SECURITY.md).
YarReader is MIT licensed; see [LICENSE](LICENSE).
